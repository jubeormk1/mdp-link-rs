...
fn main() -> ! {
    ...
    _ = board.uart_daplink.write_str("Initialising sniffer target...\n");
    ...
    _ = board.uart_daplink.write_str("Starting sniffer target...\n");
    ...
    loop {
        ...
        if let Err(error) = esb.start_tx(tx_config){
            ...
            board.leds.green.on();
            board.leds.red.off();
            _ = board.uart_daplink.write_fmt(format_args!("TX successfully "));
